...
function encodeKey(index) {
    return coder.encode(['uint32'], [index]);
}
function tryDecode(hex) {
    const result = {};
    try {
        const [u64] = coder.decode(['uint64'], hex);
        result.uint64 = u64;
    }
    catch { }
    try {
        const [u32] = coder.decode(['uint32'], hex);
        result.uint32 = u32;
    }
    catch { }
    result.bytes = hex;
...
async function testPrecompile(name, address, key, keyLabel) {
    console.log(`\n${'═'.repeat(70)}`);
