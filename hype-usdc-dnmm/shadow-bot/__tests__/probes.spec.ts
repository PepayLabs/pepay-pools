...
import { PoolConfig, PoolState, ProbeQuote, RegimeFlags } from '../types.js';
...
describe('runSyntheticProbes', () => {
  it('produces probe rows with clamp flags and regimes', async () => {
    ...
    const baseProbe = probes.find((probe) => probe.side === 'base_in') as ProbeQuote;
    const quoteProbe = probes.find((probe) => probe.side === 'quote_in') as ProbeQuote;
