...
export class ShadowBotAnalyzer {
    ...
    generateRecommendations(deltaBps, spreadBps, confBps, acceptRate, rejectRate) {
    ...
    generateHistogram(field, buckets) {
    ...
    // Identify anomalies
    findAnomalies(threshold = 3) {
        const deltaBps = this.data.map(d => d.delta_bps);
        const mean = this.mean(deltaBps);
        const stdDev = Math.sqrt(deltaBps.reduce((sum, val) => sum + Math.pow(val - mean, 2), 0) / deltaBps.length);
        return this.data.filter(d => Math.abs(d.delta_bps - mean) > threshold * stdDev ||
            d.rejection_duration > 60 ||
            d.inventory_deviation_bps > 5000);
    }
    // Export filtered data
    exportFiltered(predicate, outputPath) {
...
if (require.main === module) {
    ...
    // Generate histograms
    console.log('\n=== Delta BPS Histogram ===');
    const deltaHist = analyzer.generateHistogram('delta_bps', [10, 20, 30, 50, 75, 100, 200]);
    console.log(deltaHist);
    console.log('\n=== Spread BPS Histogram ===');
    const spreadHist = analyzer.generateHistogram('spread_bps', [5, 10, 20, 30, 50, 100]);
