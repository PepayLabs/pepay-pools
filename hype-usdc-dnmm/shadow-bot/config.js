...
async function readAddressBook(addressBookPath) {
    try {
...
function ensureHex(address, label) {
    if (!address) {
...
function parseFloatEnv(envName) {
    const value = getOptional(envName);
    if (!value)
        return undefined;
...
function resolveLabels(baseSymbol, quoteSymbol, pairLabel) {
    return {
        baseSymbol,
        quoteSymbol,
        pair: pairLabel,
...
function deriveHistogramBuckets() {
    ...
    try {
        ...
        throw new Error(`Failed to parse HISTOGRAM_BUCKETS_JSON: ${error.message}`);
    }
}
function resolveAddressBookEntry(addressBook, chainId, poolAddress) {
    if (!addressBook)
        return undefined;
    if (poolAddress) {
        const key = Object.keys(addressBook.deployments).find((deploymentKey) => addressBook.deployments[deploymentKey].poolAddress?.toLowerCase() === poolAddress.toLowerCase());
        if (key)
            return addressBook.deployments[key];
    }
