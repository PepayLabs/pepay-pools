...
interface MetricsSnapshot {
  ...
  preview_age_sec: number;
  preview_stale_reverts: number;
}
...
export class ShadowBotMonitor {
  ...
  // Alert thresholds
  private readonly ALERT_DELTA_BPS = Number(process.env.ALERT_DELTA_BPS || 100);
  private readonly ALERT_INVENTORY_DEV_BPS = Number(process.env.ALERT_INVENTORY_DEV_BPS || 2000);
  private readonly ALERT_REJECT_DURATION_SEC = Number(process.env.ALERT_REJECT_DURATION_SEC || 60);
  private readonly ALERT_SPREAD_BPS = Number(process.env.ALERT_SPREAD_BPS || 100);
  private readonly ALERT_CONF_BPS = Number(process.env.ALERT_CONF_BPS || 90);
  private readonly ALERT_PREVIEW_AGE = Number(process.env.ALERT_PREVIEW_AGE || 60);

  // Monitoring state
  private rejectStartTime?: number;
  private consecutiveRejects = 0;
  private lastAlertTime: Record<string, number> = {};
  private readonly ALERT_COOLDOWN = 300; // 5 minutes between same alerts
  private lastPreviewStaleCount = 0;
  ...
  private parseMetrics(metricsText: string): MetricsSnapshot {
  ...
  private checkDeltaAlert(metrics: MetricsSnapshot) {
  ...
  private checkSpreadAlert(metrics: MetricsSnapshot) {
  ...
  private checkConfidenceAlert(metrics: MetricsSnapshot) {
  ...
  private checkInventoryAlert(metrics: MetricsSnapshot) {
  ...
  private checkDecisionPatterns(metrics: MetricsSnapshot) {
  ...
  private checkPreviewHealth(metrics: MetricsSnapshot) {
  ...
  private addAlert(alert: Alert) {
    ...
    console.log(
      `${symbol} [${alert.level.toUpperCase()}] ${alert.type}: ${alert.message}` +
  ...
  private async sendNotification(critical: Alert[], warnings: Alert[]) {
  ...
  private async healthCheck() {
    try {
      ...
      if (this.metricsHistory.length > 2) {
        ...
        const allSame = recent.every(m =>
          m.delta_bps === recent[0].delta_bps &&
